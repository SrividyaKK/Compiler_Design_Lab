...
public:
    NFA(int nStates, int nSymbs, int nTrans) : nStates{nStates}, nSymbols{nSymbs}, nTransitions{nTrans}
    {
    ...
    void inputSymbols()
    {
        cout << "\nEnter the symbols. (Each symbols must be represented with a single character; Epsilon transitions are not allowed): ";
        char curr;
    ...
    void inputStates()
    {
        cout << "\nEnter the states. (Each state must be represented with a single character): ";
        char curr;
    ...
    {
        cout << "\nEnter transitions in the format \"<start state> <symbol> <end state>\": \n";
        char from, s, to;
        int i = 0;
